...
# List of recipients
# EMAIL_TO = ["osvsethi@abchldg.com", "akumar@abchldg.com", "borkarananta028@gmail.com"]
EMAIL_TO = ["osvsethi@abchldg.com"]
        ...
        if triggers_node:
            for trig in triggers_node[0].childNodes:
                if trig.nodeType == trig.ELEMENT_NODE:
                    ...
                    readable, logic = parse_trigger(xml_str)
                    if is_scheduled_today(logic):
                        # Append time in tuple for sorting
                        results.append((f"{subfolder_path}/{t.Name}", readable, logic.get("time")))
